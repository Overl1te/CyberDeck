...
class StreamOfferWaylandBehaviorTests(unittest.TestCase):
    def test_stream_offer_prefers_mjpeg_first_on_lan_by_default(self):
        """Validate scenario: LAN offer should prefer fast MJPEG startup before TS fallback."""
        req = SimpleNamespace(base_url="http://192.168.0.201:8080/")
        with patch.object(video_api, "require_perm", return_value=None), patch.object(
            ...
            video_api, "_mjpeg_backend_status", return_value={"native": True, "ffmpeg": True, "gstreamer": False, "screenshot": False}
        ), patch.object(
            video_api, "_mjpeg_backend_order", return_value=["native", "ffmpeg"]
        ), patch.object(
            ...
            video_api, "_is_wayland_session", return_value=False
        ), patch.object(
            video_api.os, "name", "nt"
        ), patch.dict(
        ...
        self.assertEqual((candidates[-1] or {}).get("codec"), "h265")
