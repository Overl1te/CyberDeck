...
class _WaylandBackend(_BaseInputBackend):
    ...
    def _ensure(self) -> bool:
        ...
        try:
            ...
            self._ui = UInput(events=events, name="CyberDeck Virtual Input")
            self._e = e
            return True
        except Exception as e:
    ...
    def move_rel(self, dx: int, dy: int) -> bool:
        if not self._ensure():
            return False
        self._ui.write(self._e.EV_REL, self._e.REL_X, int(dx))
        self._ui.write(self._e.EV_REL, self._e.REL_Y, int(dy))
        self._ui.syn()
        return True
    ...
    def scroll(self, dy: int) -> bool:
        if not self._ensure():
            return False
        self._ui.write(self._e.EV_REL, self._e.REL_WHEEL, int(dy))
        self._ui.syn()
        return True
