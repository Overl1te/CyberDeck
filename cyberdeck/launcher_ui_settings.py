...
def setup_settings_ui(app, ui: dict):
    ...
    header = ctk.CTkFrame(scroll, fg_color=COLOR_PANEL, corner_radius=12, height=70)
    ...
    app.sw_start_in_tray = ctk.CTkSwitch(box, text=app.tr("start_in_tray"), text_color=COLOR_TEXT)
    app.sw_start_in_tray.pack(anchor="w", padx=18, pady=(14, 6))
    app.sw_start_in_tray.select() if app.settings.get("start_in_tray") else app.sw_start_in_tray.deselect()

    app.sw_show_on_start = ctk.CTkSwitch(box, text=app.tr("show_on_start"), text_color=COLOR_TEXT)
    ...
    app.sw_close_to_tray = ctk.CTkSwitch(box, text=app.tr("close_to_tray"), text_color=COLOR_TEXT)
    ...
    app.sw_autostart = ctk.CTkSwitch(box, text=app.tr("autostart"), text_color=COLOR_TEXT)
    ...
    app.sw_hotkey = ctk.CTkSwitch(box, text=app.tr("hotkey"), text_color=COLOR_TEXT)
    ...
    app.sw_debug = ctk.CTkSwitch(box, text=app.tr("debug_logs"), text_color=COLOR_TEXT)
    app.sw_debug.pack(anchor="w", padx=18, pady=(6, 14))
    app.sw_debug.select() if app.settings.get("debug") else app.sw_debug.deselect()

    ctk.CTkLabel(box, text=app.tr("language"), text_color=COLOR_TEXT_DIM, font=FONT_SMALL).pack(
    ...
    app.opt_language.pack(fill="x", padx=18, pady=(0, 12))
    ...
    ctk.CTkLabel(box, text=app.tr("tls_https"), text_color=COLOR_TEXT_DIM, font=FONT_SMALL).pack(
        anchor="w", padx=18, pady=(8, 4)
    )

    app.sw_tls = ctk.CTkSwitch(box, text=app.tr("enable_tls"), text_color=COLOR_TEXT)
    app.sw_tls.pack(anchor="w", padx=18, pady=(0, 8))
    app.sw_tls.select() if app.settings.get("tls_enabled") else app.sw_tls.deselect()

    tls_box = ctk.CTkFrame(box, fg_color=COLOR_PANEL)
    ...
    def _row_with_browse(label: str, initial: str):
        ...
        r = ctk.CTkFrame(tls_box, fg_color=COLOR_PANEL)
        r.pack(fill="x", pady=4)
        ctk.CTkLabel(r, text=label, text_color=COLOR_TEXT).pack(side="left")
    ...
    ctk.CTkLabel(box, text=app.tr("qr_mode"), text_color=COLOR_TEXT_DIM, font=FONT_SMALL).pack(
        anchor="w", padx=18, pady=(8, 4)
    )
    app.qr_mode_var = ctk.StringVar(
        value=str(app.settings.get("qr_mode", DEFAULT_SETTINGS["qr_mode"]) or DEFAULT_SETTINGS["qr_mode"])
    )
    qr_row = ctk.CTkFrame(box, fg_color=COLOR_PANEL)
    qr_row.pack(fill="x", padx=18, pady=(0, 8))
    ...
    ctk.CTkLabel(
        box,
        text=app.tr("app_config_title", name=APP_CONFIG_FILE_NAME),
        text_color=COLOR_TEXT_DIM,
        font=FONT_SMALL,
    ).pack(anchor="w", padx=18, pady=(4, 4))

    app_cfg = ctk.CTkFrame(box, fg_color=COLOR_PANEL)
    app_cfg.pack(fill="x", padx=18, pady=(0, 10))

    cfg = dict(getattr(app, "app_config", {}) or {})

    app.sw_allow_query_token = ctk.CTkSwitch(app_cfg, text=app.tr("allow_query_token"), text_color=COLOR_TEXT)
    app.sw_allow_query_token.pack(anchor="w", pady=(0, 6))
    app.sw_allow_query_token.select() if cfg.get("allow_query_token") else app.sw_allow_query_token.deselect()

    app.sw_mdns_enabled = ctk.CTkSwitch(app_cfg, text=app.tr("mdns_enable"), text_color=COLOR_TEXT)
    app.sw_mdns_enabled.pack(anchor="w", pady=(0, 6))
    app.sw_mdns_enabled.select() if cfg.get("mdns_enabled", True) else app.sw_mdns_enabled.deselect()

    ctk.CTkLabel(app_cfg, text=app.tr("upload_max_bytes"), text_color=COLOR_TEXT_DIM, font=FONT_SMALL).pack(
        anchor="w", pady=(6, 2)
    )
    app.ent_upload_max_bytes = ctk.CTkEntry(app_cfg)
    app.ent_upload_max_bytes.pack(fill="x")
    app.ent_upload_max_bytes.insert(0, str(cfg.get("upload_max_bytes", 0)))

    ctk.CTkLabel(app_cfg, text=app.tr("upload_allowed_ext"), text_color=COLOR_TEXT_DIM, font=FONT_SMALL).pack(
        anchor="w", pady=(8, 2)
    )
    app.ent_upload_allowed_ext = ctk.CTkEntry(app_cfg)
    app.ent_upload_allowed_ext.pack(fill="x")
    app.ent_upload_allowed_ext.insert(0, str(cfg.get("upload_allowed_ext", "")))

    app.sw_verbose_http_log = ctk.CTkSwitch(app_cfg, text=app.tr("verbose_http"), text_color=COLOR_TEXT)
    app.sw_verbose_http_log.pack(anchor="w", pady=(10, 4))
    app.sw_verbose_http_log.select() if cfg.get("verbose_http_log", True) else app.sw_verbose_http_log.deselect()

    app.sw_verbose_ws_log = ctk.CTkSwitch(app_cfg, text=app.tr("verbose_ws"), text_color=COLOR_TEXT)
    app.sw_verbose_ws_log.pack(anchor="w", pady=4)
    app.sw_verbose_ws_log.select() if cfg.get("verbose_ws_log", True) else app.sw_verbose_ws_log.deselect()

    app.sw_verbose_stream_log = ctk.CTkSwitch(app_cfg, text=app.tr("verbose_stream"), text_color=COLOR_TEXT)
    app.sw_verbose_stream_log.pack(anchor="w", pady=(4, 10))
    app.sw_verbose_stream_log.select() if cfg.get("verbose_stream_log", True) else app.sw_verbose_stream_log.deselect()

    btn_row = ctk.CTkFrame(box, fg_color=COLOR_PANEL)
    ...
    CyberBtn(box, text=app.tr("apply"), command=app.save_settings_action, height=34).pack(
        padx=18, pady=(4, 10), fill="x"
    )

    app.lbl_settings_status = ctk.CTkLabel(box, text="", font=FONT_SMALL, text_color=COLOR_TEXT_DIM)
