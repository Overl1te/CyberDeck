...
from .video_ffmpeg import _ffmpeg_mjpeg_stream
from .video_wayland import _gst_mjpeg_stream, _grim_mjpeg_stream, _wayland_grim_frame, _wayland_screenshot_tool_frame
...
def _lowlat_bitrate_cap_k(max_w: int, fps: int, codec: str = "h264") -> int:
    ...
    base = 3200.0 * (w / 1280.0) * (f / 30.0)
    if str(codec or "").lower() == "h265":
        base *= 0.75
    cap = int(round(base))
    return max(900, min(7000, cap))
...
def _mjpeg_backend_status(monitor: int, fps: int, probe: bool = False) -> Dict[str, bool]:
    ...
    ffmpeg_enabled = os.environ.get("CYBERDECK_DISABLE_FFMPEG_MJPEG", "0") != "1"
    ...
    wayland_pref_non_x11grab = os.environ.get("CYBERDECK_WAYLAND_PREFER_NON_X11GRAB", "1") == "1"
    force_x11grab = os.environ.get("CYBERDECK_FORCE_WAYLAND_X11GRAB", "0") == "1"
    if (
        ffmpeg_ok
        and os.name != "nt"
        and _is_wayland_session()
...
def _mjpeg_stream_for_backend(
    ...
    *,
    ...
    if name == "ffmpeg":
        if os.environ.get("CYBERDECK_DISABLE_FFMPEG_MJPEG", "0") == "1":
            return None
