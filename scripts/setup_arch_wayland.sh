...
DESKTOP_HINT="$(printf '%s %s %s' "${XDG_CURRENT_DESKTOP:-}" "${DESKTOP_SESSION:-}" "${GDMSESSION:-}" | tr '[:upper:]' '[:lower:]')"
PREFER_GST_GNOME=0
if [[ "${DESKTOP_HINT}" == *gnome* && "${GST_CAPTURE_OK}" -eq 1 ]]; then
  PREFER_GST_GNOME=1
fi

if [[ "${PREFER_GST_GNOME}" -eq 1 ]]; then
  ORDER="gstreamer,ffmpeg,screenshot,native"
elif [[ "${FFMPEG_PIPEWIRE_OK}" -eq 1 ]]; then
  ORDER="ffmpeg,gstreamer,screenshot,native"
