...
echo "[2/7] Checking ffmpeg PipeWire input support..."
if ffmpeg -hide_banner -formats 2>/dev/null | grep -qi 'pipewire'; then
  echo "      OK: ffmpeg supports pipewire."
else
  echo "      WARN: current ffmpeg has no pipewire input."
  if command -v yay >/dev/null 2>&1; then
    echo "      Trying to install ffmpeg-full via yay..."
    yay -S --needed --noconfirm ffmpeg-full
  elif command -v paru >/dev/null 2>&1; then
    echo "      Trying to install ffmpeg-full via paru..."
    paru -S --needed --noconfirm ffmpeg-full
  else
    echo "      No AUR helper found (yay/paru)."
    echo "      Install an ffmpeg build with --enable-libpipewire manually, then rerun this script."
  fi
fi

echo "[3/7] Verifying ffmpeg PipeWire support after install..."
if ffmpeg -hide_banner -formats 2>/dev/null | grep -qi 'pipewire'; then
  echo "      OK: pipewire input is available."
else
  echo "[ERR] ffmpeg still does not support pipewire input."
  echo "      Video stream on Wayland will not work until ffmpeg has pipewire support."
  exit 1
fi
